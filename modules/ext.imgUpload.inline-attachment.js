...
/*
 * Inline Text Attachment (mediawiki modification)
 *
 * Author: Roy van Kaathoven
 * Contact: ik@royvankaathoven.nl
 *
 * Modificator: usbport11
 * Contact: none
 */
(function(document, window) {
  ...
  inlineAttachment.prototype.uploadFile = function(file) {  
    ...
    //mediawiki data (can read here: https://www.mediawiki.org/wiki/API:Upload)
    formData.append("action", "upload");
    formData.append("token", mw.user.tokens.get('editToken')); //can read here: https://doc.wikimedia.org/mediawiki-core/master/js/#!/api/mw.user
  ...
  inlineAttachment.prototype.onFileUploadResponse = function(xhr) { 
    if (this.settings.onFileUploadResponse.call(this, xhr) !== false) {
	  ...
	  //analyze mediawiki JSON result (console.log(result);)
	  //if we get error
	  if(result.error) {
		var newValue = "Error: " + result.error.info;
	  ...
	  if(result.upload.result == "Warning") {
		...
		var warnging_text;
		for (var Warning_Type in result.upload.warnings) {
		  last_waring_name = Warning_Type;
		}
		var newValue = "Warning: " + last_waring_name;
		var text = this.editor.getValue().replace(this.lastValue, newValue);
		this.editor.setValue(text);
		return;
	  }
	  ...
	  if(result.upload.result == "Success") {
		...
		var filename;
		var text;
		//get filename from wiki upload parameter (JSON)
		filename = result.upload.filename;
		//replace string {filename} to new uploaded file name string
		newValue = this.settings.urlText.replace(this.filenameTag, filename);
		//replace [last value] in editor to uploaded file name string (lastvalue variable changing by process run)
		text = this.editor.getValue().replace(this.lastValue, newValue);
		//replace text in wiki editor
        this.editor.setValue(text);
		//call result function -> empty in this case
